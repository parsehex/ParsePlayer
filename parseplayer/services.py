...
def _file_needs_update(src: Path, dst: Path) -> bool:
	...
	try:
		st_src = src.stat()
		st_dst = dst.stat()
		# On legacy devices like the PEARL, timestamps are often unreliable.
		# We'll rely on file size matches to detect unchanged files.
		return st_src.st_size != st_dst.st_size
	except OSError:
		return True


def derive_virtual_artist_album(track_path: str,
                                music_root: str) -> tuple[str, str]:
...
def fetch_usb_devices(db):
	...
	for row in rows:
		...
		if live:
			item["label"] = live.get("display_name") or live.get(
			    "label") or item["label"]
			...
			item["device_path"] = item["device_uuid"] if str(
			    item["device_uuid"]).startswith("/dev/") else "-"
			item["size_human"] = "-"
...
def build_track_browse_groups(
	...
	for track in tracks:
		...
		artist_folder_path = str(
		    music_root_path /
		    artist_key) if artist_key != "__root__" else str(music_root_path)

		artist_totals[artist_key]["label"] = artist_label
		artist_totals[artist_key]["folder_path"] = artist_folder_path
		artist_totals[artist_key]["total"] = int(
		    artist_totals[artist_key]["total"]) + 1
		artist_totals[artist_key]["selected"] = int(
		    artist_totals[artist_key]["selected"]) + selected
		track_size = int(track.get("size_bytes") or 0)
		artist_totals[artist_key]["size_bytes"] = int(
		    artist_totals[artist_key]["size_bytes"]) + track_size
		if selected:
			artist_totals[artist_key]["selected_size_bytes"] = int(
			    artist_totals[artist_key]["selected_size_bytes"]) + track_size

		album_key = parts[1] if len(parts) >= 3 else ""
		album_label = album_key
		album_folder_path = str(Path(artist_folder_path) /
		                        album_key) if album_key else ""
		...
		if active_artist and artist_key == active_artist and album_key:
			...
			album_totals[album_key]["total"] = int(
			    album_totals[album_key]["total"]) + 1
			album_totals[album_key]["selected"] = int(
			    album_totals[album_key]["selected"]) + selected
			album_totals[album_key]["size_bytes"] = int(
			    album_totals[album_key]["size_bytes"]) + track_size
			if selected:
				album_totals[album_key]["selected_size_bytes"] = int(
				    album_totals[album_key]["selected_size_bytes"]) + track_size
	...
	artist_groups = [{
	    ...
	    int(data["selected"]),
	    "size_human":
	    format_bytes(int(data["size_bytes"])),
	    "selected_size_human":
	    format_bytes(int(data["selected_size_bytes"])),
	    "is_active":
	    key == active_artist,
	...
	filtered_tracks.sort(key=lambda track: (
	    -int(track["selected_for_sync"]),
	    (track["artist"] or "").lower(),
	    str(track["title"]).lower(),
	))

	if active_artist and all(group["key"] != active_artist
	                         for group in artist_groups):
		active_artist = ""
		active_album = ""
		filtered_tracks = list(tracks)

	if active_album and active_artist and all(group["key"] != active_album
...
def run_import_library_input(app):
	with app.app_context():
		...
		if not source_device:
			set_job_state(
			    "error",
			    "No USB marked as library_input. Detect devices and assign one.")
			return
		...
		if not raw_mount or not Path(raw_mount).is_dir():
			...
			if not raw_mount:
				set_job_state(
				    "error",
				    f"Could not mount '{source_device['label']}'. Is it still plugged in?"
				)
				return
			queries.update_usb_device_mount(db,
			                                device_identifier,
			                                raw_mount,
			                                by_uuid=True)
			db.commit()
		...
		if not mount_path.exists() or not mount_path.is_dir():
			set_job_state(
			    "error",
			    f"Library input USB mount path is unavailable: {mount_path}")
			return
		...
		source_files = [
		    f for f in mount_path.rglob("*")
		    if f.is_file() and f.suffix.lower() in SUPPORTED_EXTENSIONS
		]
		total_files = len(source_files)

		if total_files == 0:
			set_job_state("completed", "No supported music files found on USB.", 100,
			              0, 0)
			return
		...
		try:
			for source_file in source_files:
				...
				processed_files += 1
				if job_state["status"] == "stopping":
					set_job_state("idle", "Import cancelled by user.")
					return

				pct = int((processed_files / total_files) * 100)
				set_job_state("running", f"Importing {source_file.name}...", pct,
	...
	with app.app_context():
		...
		if not raw_mount or not Path(raw_mount).is_dir():
			...
			if not raw_mount:
				set_job_state(
				    "error",
				    f"Could not mount '{backup_device['label']}'. Is it still plugged in?"
				)
				return
			queries.update_usb_device_mount(db,
			                                device_identifier,
		...
		if not mount_path.exists() or not mount_path.is_dir():
			set_job_state("error",
			              f"Backup USB mount path is unavailable: {mount_path}")
			return

		library_root = Path(app.config["MUSIC_ROOT"])
		if not library_root.exists() or not library_root.is_dir():
			set_job_state("error",
			              f"Library source folder is unavailable: {library_root}")
			return
		...
		try:
			for source_file in source_files:
				...
				processed_files += 1
				if job_state["status"] == "stopping":
					set_job_state("idle", "Backup cancelled by user.")
					return
				pct = int((processed_files / total_files) * 100)
				set_job_state("running", f"Backing up {source_file.name}...", pct,
				              processed_files, total_files)
	...
	with app.app_context():
		...
		if not raw_mount or not Path(raw_mount).is_dir():
			...
			if not raw_mount:
				set_job_state(
				    "error",
				    f"Could not mount '{mp3_device['label']}'. Is it still plugged in?"
				)
				return
			queries.update_usb_device_mount(db,
			                                device_identifier,
		...
		if not mount_path.exists() or not mount_path.is_dir():
			set_job_state("error",
			              f"MP3 player USB mount path is unavailable: {mount_path}")
			return

		sync_root = mount_path
		library_root = Path(app.config["MUSIC_ROOT"])

		queries.log_sync_run(db, "sync_mp3", "started",
		                     f"Starting sync of {selected_count} tracks.")
		db.commit()
		...
		for f in sync_root.rglob("*"):
			...
			if job_state["status"] == "stopping":
				set_job_state("idle", "Sync cancelled by user.")
				return
				continue
		...
		try:
			...
			for track in selected_tracks:
				...
				if total_files > 0:
					...
					pct = 0
				set_job_state("running", f"Syncing {source_file.name}...", pct,
				              processed_files, total_files)
